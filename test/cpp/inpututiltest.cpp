...
TEST(ApplyCurveTest, ZeroInputReturnsZero) {
    EXPECT_DOUBLE_EQ(indy::applyCurve(0.0, kDeadband, kExponent), 0.0);
}

TEST(ApplyCurveTest, WithinDeadbandReturnsZero) {
    EXPECT_DOUBLE_EQ(indy::applyCurve(0.05, kDeadband, kExponent), 0.0);
    EXPECT_DOUBLE_EQ(indy::applyCurve(-0.05, kDeadband, kExponent), 0.0);
}

TEST(ApplyCurveTest, AtDeadbandEdgeReturnsZero) {
    // The curve starts AT the deadband — the edge itself must still be 0
    EXPECT_DOUBLE_EQ(indy::applyCurve(kDeadband, kDeadband, kExponent), 0.0);
    EXPECT_DOUBLE_EQ(indy::applyCurve(-kDeadband, kDeadband, kExponent), 0.0);
}
...
TEST(ApplyCurveTest, FullDeflectionReturnsOne) {
    EXPECT_NEAR(indy::applyCurve(1.0, kDeadband, kExponent), 1.0, kTolerance);
    EXPECT_NEAR(indy::applyCurve(-1.0, kDeadband, kExponent), -1.0, kTolerance);
}

TEST(ApplyCurveTest, FullDeflectionReturnsOneWithCubicExponent) {
    EXPECT_NEAR(indy::applyCurve(1.0, kDeadband, 3.0), 1.0, kTolerance);
    EXPECT_NEAR(indy::applyCurve(-1.0, kDeadband, 3.0), -1.0, kTolerance);
}
...
TEST(ApplyCurveTest, FullDeflectionReturnsOneWithLinearExponent) {
    ...
    EXPECT_NEAR(indy::applyCurve(1.0, kDeadband, 1.0), 1.0, kTolerance);
    EXPECT_NEAR(indy::applyCurve(-1.0, kDeadband, 1.0), -1.0, kTolerance);
}
...
TEST(ApplyCurveTest, NegativeInputMirrorsPositive) {
    const double pos = indy::applyCurve(0.6, kDeadband, kExponent);
    const double neg = indy::applyCurve(-0.6, kDeadband, kExponent);
...
TEST(ApplyCurveTest, OutputIsMonotonicallyIncreasing) {
    ...
    for (int i = 1; i <= 10; ++i) {
        ...
        const double output = indy::applyCurve(input, kDeadband, kExponent);
...
TEST(ApplyCurveTest, CurveIsBelow_LinearMidpoint_WithQuadraticExponent) {
    ...
    const double curvedOutput = indy::applyCurve(mid, kDeadband, kExponent);    // quadratic
    const double linearOutput  = indy::applyCurve(mid, kDeadband, 1.0);          // linear
