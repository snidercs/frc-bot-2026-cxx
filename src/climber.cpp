...
Climber::Climber() {
    ...
    // Default command: always retract unless overridden (e.g. by extendCommand).
    SetDefaultCommand(Run([this] { setDutyCycle(kRetractDutyCycle); }).WithName("ClimberDefault"));
}
...
// Command factories
frc2::CommandPtr Climber::retractCommand() {
    return Run([this] { setDutyCycle(kRetractDutyCycle); })
        .WithName("Retract")
        .FinallyDo([this] { stop(); });
}

frc2::CommandPtr Climber::extendCommand() {
    return Run([this] { setDutyCycle(kExtendDutyCycle); })
        .WithName("Extend")
        .FinallyDo([this] { stop(); });
}
