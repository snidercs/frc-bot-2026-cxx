...
/** Horizontal shaker subsystem — spins a single TalonFX motor at a fixed duty cycle.
 
    Runs in reverse relative to the vertical shaker to agitate game pieces
    horizontally through the uptake path. Intended to run concurrently with
    the shooter via `.AlongWith()`.
...
*/
class HorizontalShaker : public frc2::SubsystemBase {
public:
    HorizontalShaker();
...
private:
    ctre::phoenix6::hardware::TalonFX _motor{
        config::integer("horizontal_shaker_device_id"),
        config::str("horizontal_shaker_can_bus")};
